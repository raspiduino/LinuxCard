...
//loopback unimplemented because fuck it
...
static void dz11prvTx(uint_fast8_t val)		//assumes it will be followed by recalc
{
	...
	else if (!gDZ11.enabled)
		err_str("DZ11: write while fisabled");
	else {
...
static bool dz11prvRealMemAccess(uint_fast8_t regIdx, uint16_t *valP, bool write)
{
	uint_fast8_t lineNo;
	uint_fast16_t v;
...
static bool dz11prvMemAccess(uint32_t pa, uint_fast8_t size, bool write, void* buf, void* userData)
{
	(void)userData;
	
	pa &= 0x00ffffff;

		if (size != 2 || (pa & 7) || pa > 0x18)
		return false;
	
	return dz11prvRealMemAccess(pa / 8, buf, write);
}
...
bool dz11init(void)
{
	...
	return memRegionAdd(0x1c000000, 0x01000000, dz11prvMemAccess, (void*)0);
}
